...
void fill_factor(double * m, SEXP v, int * pos, int addNA);
void fill_double(double * m, SEXP v, int * pos, int addNA);
void fill_integer(double * m, SEXP v, int * pos, int addNA);
void fill_logical(double * m, SEXP v, int * pos, int addNA);

void fill_factor(double *m, SEXP v, int * pos, int addNA) {

  int nlvls = LENGTH(GET_LEVELS(v));
  ...
  for (int i = 0; i < LENGTH(v); i++) {
    if (!addNA) {
      ...
      if (INTEGER(v)[i] == NA_INTEGER) {
        m[i + (*pos + nlvls) * LENGTH(v)] = 1;
      } else {
        m[i + (*pos + (INTEGER(v)[i] - 1)) * LENGTH(v)] = 1;
      }
    }
  }
  *pos += LENGTH(GET_LEVELS(v)) + addNA;
}
...
SEXP predict_onehot(SEXP onehot, SEXP data) {
  ...
  int addNA = INTEGER(Rf_getAttrib(onehot, install("addNA")))[0];

  //Rprintf("addNA_value: %d\n", addNA);
  ...
  for (int i = 0; i < LENGTH(onehot); i++) {
    ...
    if (IS_FACTOR(onehot, i)) {
      fill_factor(m, v, &pos, addNA);

    } else if (Rf_isInteger(v)) {
      fill_integer(m, v, &pos, addNA);

    } else if (Rf_isReal(v)) {
      fill_double(m, v, &pos, addNA);

    } else if (Rf_isLogical(v)) {
      fill_logical(m, v, &pos, addNA);
    }
  }
